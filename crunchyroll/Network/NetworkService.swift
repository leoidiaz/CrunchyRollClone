...
class NetworkService {
      ...
      private let trendingKey = "trending"
      private let subKey = "anime"
    //https://kitsu.io/api/edge/anime?filter[text]=demon%20slayer
      private let queryKey = "filter[text]"
    //https://kitsu.io/api/edge/anime?page[limit]=5&page[offset]=12062

     func fetchAnimes(searchType: AnimeFetchType, query: String?, completion: @escaping(Result<Data,CRError>) -> Void) {
      ...
      func fetchMyListAnime(idURL: String?, completion: @escaping(Result<Data, CRError>) -> Void) {
        guard let baseURL = baseURL else { return completion(.failure(.invalidURL)) }
      ...
      func fetchPoster(posterPath: String, completion: @escaping(Result<Data, CRError>) -> Void){
        guard let posterURL = URL(string: posterPath) else { return completion(.failure(.invalidURL))}
        let request = URLRequest(url: posterURL)
        resumeDataTask(withRequest: request, completion: completion)
    }
    
      func resumeDataTask(withRequest request: URLRequest, completion: @escaping(Result<Data, CRError>) -> Void){
